defmodule Indexer.Fetcher.Cosmos.DashboardAPIClient do
  ...
  @doc """
  Fetches historical transactions for catchup sync.
  Returns transactions ordered by block height ASC starting from the given height.

  ## Parameters
  - `from_height` - Block height to start from (default: 0)
  - `limit` - Maximum number of transactions to fetch (default: 100, max: 500)

  ## Returns
  - `{:ok, %{transactions: list(), sync: map()}}`: Transactions and sync metadata
  - `{:error, reason}`: Error tuple
  """
  @spec fetch_historical_transactions(non_neg_integer(), pos_integer()) ::
          {:ok, %{transactions: list(map()), sync: map()}} | {:error, term()}
  def fetch_historical_transactions(from_height \\ 0, limit \\ 100) do
    url = build_url("/api/v1/transactions/historical", %{
      from_height: from_height,
      limit: min(limit, 500)
    })

    case do_get_with_sync(url) do
      {:ok, transactions, sync} ->
  ...
  @doc """
  Fetches the current sync status from the dashboard indexer.

  ## Returns
  - `{:ok, map()}`: Sync status with min/max heights and transaction count
  - `{:error, reason}`: Error tuple
  """
  @spec fetch_sync_status() :: {:ok, map()} | {:error, term()}
  def fetch_sync_status do
    url = build_url("/api/v1/transactions/sync-status")
    do_get(url)
  end
  ...
  # Variant of do_get that also extracts sync metadata from the response
  defp do_get_with_sync(url) do
    headers = [
      {"Content-Type", "application/json"},
      {"Accept", "application/json"}
    ]

    Logger.debug("Fetching from Dashboard API with sync: #{url}")

    case HTTPoison.get(url, headers, recv_timeout: @default_timeout, timeout: @default_timeout) do
      {:ok, %Response{body: body, status_code: 200}} ->
        case Helper.decode_json(body) do
          %{"success" => true, "data" => data, "sync" => sync} ->
            {:ok, data, sync}
